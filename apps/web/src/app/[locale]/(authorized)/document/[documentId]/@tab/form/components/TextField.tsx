...
import FieldInputIcn from "@icons/field-input.svg";
...
import CanvasFeildTextarea, { CanvasFeildTextareaProps } from "../../../classes/form/CanvasFeildTextarea";
...
const TextField = () => {
  ...
  const [fieldName, setFieldName] = useState(`text-field-1`);
  const [isFieldMultiline, setIsFieldMultiline] = useState(false);

  const handleAddField = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const required = formData.get("required") === "on";
    const allowMultiline = formData.get("allow_multiline") === "on";
    ...
    documentManager.addFormField(allowMultiline ? CanvasFeildTextarea.type : CanvasFeildInput.type, {
      required,
      fieldName,
      fieldPlaceholder,
      fontSize,
      ...
      width,
      height,
    } as CanvasFeildTextareaProps | CanvasFeildTextareaProps);

    setFieldName(
      `text-field-${documentManager.currentCanvas.getObjects().filter(({ type }) => type === "canvas-field-input" || type === "canvas-field-textarea").length + 1}`
    );
  };
  ...
  return (
    <form onSubmit={handleAddField} className="bg-primary-100/10 flex flex-col gap-4 px-3 py-2.5 rounded-lg">
      <div className="flex items-center gap-2.5">
        <FieldInputIcn className="size-6.5" />
        <h5 className="grow text-base text-center">Text field</h5>
        <dialog
          popover="auto"
        ...
        >
          ...
          <FileldSettingsModal
            fieldType={isFieldMultiline ? CanvasFeildTextarea.type : CanvasFeildInput.type}
      ...
      <div className="flex gap-2 justify-evenly">
        ...
        <label className="bg-primary-100/10 flex items-center gap-2 text-xs px-2.5 py-1 rounded-xl hover:bg-primary-100/20 active:text-primary-500">
          <input
            ...
            checked={isFieldMultiline}
            onChange={(e) => setIsFieldMultiline(e.target.checked)}
          />
          Allow multiline
        </label>
      </div>
