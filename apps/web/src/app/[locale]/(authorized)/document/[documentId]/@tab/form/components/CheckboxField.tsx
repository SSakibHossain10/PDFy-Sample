...
import FieldCheckboxIcn from "@icons/field-checkbox.svg";
import FieldSettingsIcn from "@public/icons/doutone/field-settings.svg";
import { CSSProperties, useEffect, useState } from "react";
import { BiPlus } from "react-icons/bi";
import CanvasFeildCheckbox, { CanvasFeildCheckboxProps } from "../../../classes/form/CanvasFeildCheckbox";
...
const CheckboxField = () => {
  ...
  const [fieldName, setFieldName] = useState(`checkbox-field-1`);
  ...
  const handleAddField = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    documentManager.addFormField(CanvasFeildCheckbox.type, {
      fieldName,
      fieldBorderWidth,
      fieldBorderColor,
      fill,
      ...
      width,
      height,
    } as CanvasFeildCheckboxProps);

    setFieldName(
      `checkbox-field-${documentManager.currentCanvas.getObjects().filter(({ type }) => type === "canvas-field-checkbox").length + 1}`
    );
  };
  ...
  return (
    <form onSubmit={handleAddField} className="bg-primary-100/10 flex flex-col gap-4 px-3 py-2.5 rounded-lg">
      <div className="flex items-center gap-2.5">
        <FieldCheckboxIcn className="size-6.5" />
        <h5 className="grow text-base text-center">Checkbox field</h5>
        <dialog
          popover="auto"
        ...
        >
          ...
          <FileldSettingsModal fieldType={CanvasFeildCheckbox.type} myPopover={fieldSettingsPopover} />
        </dialog>
        <button
          type="button"
