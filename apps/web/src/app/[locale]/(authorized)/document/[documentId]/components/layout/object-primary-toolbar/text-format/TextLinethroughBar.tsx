...
import LinethroughIcn from "@icons/doutone/linethrough.svg";
...
const TextLinethroughBar = ({ parentPopover }: { parentPopover: IPopover<HTMLDivElement | HTMLDialogElement> }) => {
  const [isTextLinethroughd, setIsTextLinethroughd] = useState<CanvasTextbox["linethrough"]>(
    false as unknown as CanvasTextbox["linethrough"]
  );
  ...
  const setStateHandler = () => {
    ...
    if (OBJECTS_SHOULD_ACTIVE.includes(activeObject?.type as (typeof OBJECTS_SHOULD_ACTIVE)[number])) {
      setIsTextLinethroughd((activeObject as CanvasTextbox).linethrough || false);
    }
  };
  ...
  const handleSetTextLinethroughd = (textLinethroughd: CanvasTextbox["linethrough"]) => {
    setIsTextLinethroughd(textLinethroughd);
    documentManager.changeObjectLinethrough(textLinethroughd);
  };

  return (
    <>
      <button
        aria-describedby="linethrough-tooltip"
        ...
        style={{ anchorName: "--linethrough-anchor" } as CSSProperties}
        onClick={() => handleSetTextLinethroughd(!isTextLinethroughd)}
      >
        <LinethroughIcn className="size-5" />
        <span className="sm:hidden text-2xs">Linethrough</span>
      </button>
      {/* linethrough-tooltip */}
      <div
        id="linethrough-tooltip"
        ...
        style={
          {
            positionAnchor: "--linethrough-anchor",
          } as CSSProperties
        }
      >
...
export default TextLinethroughBar;
