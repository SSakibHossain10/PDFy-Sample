...
interface layerObjectType extends FabricObject {
  index: number;
}

function LayerOverlappingPanel({ parentPopover }: { parentPopover: IPopover<HTMLDialogElement> }) {
  const [layerObjects, setLayerObjects] = useState<{ svg: string; object: layerObjectType; isActiveObj: boolean }[]>(
    []
  );
  ...
  const setStateHandler = () => {
    ...
    (async () => {
      Promise.all(
        //@ts-expect-error
        (documentManager.currentCanvas.getObjects() as layerObjectType[])
          .map((object, index) => {
            object.index = index;
            return object;
          }) //@ts-expect-error
          .filter((object) => documentManager.isObjectOverLaping(object))
          .reverse()
          .map(async (object) => {
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, dropIndex: number, objectDropIndex: number) => {
    e.preventDefault();
    ...
    //@ts-expect-error
    documentManager.changeObjectLayerOrder(draggedObject.object, layerObjects.length - objectDropIndex);
  ...
  return (
    <div className="grow w-full min-h-42 flex flex-col gap-3 pt-3 sm:pt-4 -mb-5 overflow-auto relative">
      {/* top dropable area */}
      <div
        ...
        onDrop={(e) => handleDrop(e, 0, layerObjects[0].object.index)}
      >
      ...
      {layerObjects.map(({ svg, object, isActiveObj }, indx) => (
        <div className="shrink-0 w-full h-13 px-2 sm:px-4 relative" key={indx}>
          ...
          <div
            ...
            onDragLeave={handleDropLeave}
            onDragOver={handleDropOver}
            onDrop={(e) => handleDrop(e, indx + 1, object.index + 1)}
          >
            <div className="w-full h-1 bg-primary-500" />
          </div>
        </div>
...
export default LayerOverlappingPanel;
