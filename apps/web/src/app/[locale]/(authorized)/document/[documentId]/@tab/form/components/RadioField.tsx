...
import FieldRadioIcn from "@icons/field-radio.svg";
import FieldSettingsIcn from "@public/icons/doutone/field-settings.svg";
import { CSSProperties, useEffect, useState } from "react";
import { BiPlus } from "react-icons/bi";
import CanvasFeildRadio, { CanvasFeildRadioProps } from "../../../classes/form/CanvasFeildRadio";
...
const RadioField = () => {
  ...
  const [fieldName, setFieldName] = useState(`radio-field-1`);
  ...
  const handleAddField = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    documentManager.addFormField(CanvasFeildRadio.type, {
      required,
      fieldName,
      fieldBorderWidth,
      fieldBorderColor,
      ...
      width,
      height,
    } as CanvasFeildRadioProps);
  };

  const resetFeildName = () => {
    setFieldName(
      `radio-field-${documentManager.currentCanvas.getObjects().filter(({ type }) => type === "canvas-field-radio").length + 1}`
    );
  };
  ...
  return (
    <form onSubmit={handleAddField} className="bg-primary-100/10 flex flex-col gap-4 px-3 py-2.5 rounded-lg">
      <div className="flex items-center gap-2.5">
        <FieldRadioIcn className="size-6.5" />
        <h5 className="grow text-base text-center">Radio field</h5>
        <dialog
          popover="auto"
        ...
        >
          ...
          <FileldSettingsModal fieldType={CanvasFeildRadio.type} myPopover={fieldSettingsPopover} />
        </dialog>
        <button
          type="button"
